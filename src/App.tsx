...
export const App: React.FC = () => {
  const { activeTab, setActiveTab, candidates, papers, venues, clearAllData } = useExamStore();
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col selection:bg-indigo-100 selection:text-indigo-900">
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                title="Wipe current browser data"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 hover:border-rose-300 text-slate-500 hover:text-rose-600 text-xs font-medium rounded-lg transition-colors cursor-pointer"
              >
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center gap-3 text-rose-600">
              <AlertCircle className="w-6 h-6" />
              <h3 className="font-bold text-slate-900 text-base">Reset All Workspace Data?</h3>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed">
              This will clear all ingested candidates, access arrangements, exam papers, venues, and seating allocations from your browser's local storage. This action cannot be undone.
            </p>
